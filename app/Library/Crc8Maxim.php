...
{
    protected $Init = 0x0;
    protected $XorOut = 0x0;
    protected $RefOut = TRUE;
    protected $Poly = 0x31;
    protected $RefIn = TRUE;
    protected $Check = 0xA1;
    protected $Table = [
    ...
    public function ComputeCrc($string) {
        $crc = $this->Init;

        $data = $this->processStr($string);

        foreach ($data as $d) {
            $crc = $this->Table[$d ^ $crc];
        }

        $crc = $crc ^ $this->XorOut;

        return $this->ReverseCrc($crc & 0xFF);
    }

    protected function ReverseCrc($crc)
    {
        $result = str_split(dechex($crc),1);

        return sprintf('%02s%02s', $result[1], $result[0]);
    }

    protected function processStr($string)
    {
        return array_map('hexdec',str_split(str_replace(' ', '', $string),2));
    }
}