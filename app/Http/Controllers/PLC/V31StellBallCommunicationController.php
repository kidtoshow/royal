...
class V31StellBallCommunicationController extends Controller
{
    // 七段顯示器
    // 轉分
    ...
    public function displayTurnPoint($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v31_stell_ball_display_turn_point');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function displayScore($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_display_score');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function displayOpenPoint($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_display_open_point');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function displayAllStatus($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_display_get_all');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function plcTurnPointAmount($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_plc_turn_point_amount');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function plcBigRewardStatus($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_plc_big_reward_status');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function plcSmallRewardStatus($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_plc_small_reward_status');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function plcPush($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_plc_push');
            logger('ip:'.$ip);
            logger('port:'.$port);
            logger('指令：'.self::PUSH);
            logger('回傳值：'.$return);
    ...
    public function plcDownTurn($ip, $port){
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_plc_down_turn');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function plcUpTurn($ip, $port){
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_plc_up_turn');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function plcStartOrStop($ip, $port){
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_plc_start_stop');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function plcSubPoint($ip, $port){
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_plc_sub_point');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function plcWashPoint($ip, $port){
        ...
        while(!$result_on && $count_on < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_plc_wash_point_on');
            logger('ip:'.$ip);
            logger('port:'.$port);
        ...
        while(!$result_off && $count_off < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_plc_wash_point_off');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function plcControlOpenPointAmount($ip, $port, $amount){
        ...
        while(!$amount_result && $amount_count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_plc_open_point_amount');
            logger('ip:'.$ip);
            logger('port:'.$port);
            logger('指令：'.$str.$crc);
            logger('回傳值：'.$return);
        ...
        while(!$speed_result && $speed_count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_plc_open_point_speed');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function plcControlOpenPoint($ip, $port){
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_plc_open_point');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function rewardSwitch($ip, $port){
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_reward_switch');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function plcPakOpenPointVerify($ip, $port, $amount)
    {
        ...
        while(!$result && $count < 5) {
            ...
            pclose($handle);
            logger('v31_stell_ball_pak_open_point_verify');
            logger('ip:'.$ip);
            logger('port:'.$port);
            logger('次數：'.$amount);
            logger('回傳值：'.$return);
    ...
    public function plcOpenPointStatus($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_open_point_status');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function plcDownTurnLong($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::WASH_POINT_CONTROL);
            logger('v31_stell_ball_plc_down_turn_long');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    {
        logger('v31_stell_ball_pak_open_verify_v2');
        logger('ip:'.$ip);
