...
class V120SlotController extends Controller
{
    //取值韋峯部分
    // 洗分控制
    CONST WASH_POINT_CONTROL = '01 05 10 69 FF 00 58 E6';
    // 壓分
    CONST OPEN_POINT_AMOUNT = '01 06 00 F1 %04s';
    CONST OPEN_POINT_SPEED = '01 06 01 0F 00 0A 38 32';
    // 開分控制
    CONST OPEN_POINT_CONTROL = '01 05 10 75 FF 00 99 20';
    // 退幣ＯＮ
    CONST REFUND_ON = '01 05 10 56 FF 00 68 EA';
    // 退幣ＯＦＦ
    CONST REFUND_OFF = '01 05 10 56 00 00 29 1A';
    // 投幣ＯＮ
    CONST COIN_ON = '01 05 10 60 FF 00 88 E4';
    // 投幣ＯＦＦ
    CONST COIN_OFF = '01 05 10 60 00 00 C9 14';
    // 大獎狀態
    ...
    // 大賞燈切換
    CONST FLASH_1_ON = '01 05 10 04 FF 00 C9 3B';
    // 大閃燈key1_OFF
    CONST FLASH_1_OFF = '01 05 08 18 00 00 4F AD';
    // CHANCE
    CONST FLASH_2_ON = '01 05 10 2C FF 00 49 33';
    ...
    public function washPointControl($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_20solt_wash_point_control');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function openPointControl($ip, $port)
    {
        ...
        while (!$result && $count < 5) {
            ...
            logger('v1_20solt_open_point_control');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function openPointStatus($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_20solt_open_point_status');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function openPointZero($ip, $port)
    {
        ...
        while (!$result && $count < 5){
            ...
            logger('v1_20solt_open_point_zero');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function checkAutoCard($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_20solt_eheck_auto_card'.PHP_EOL);
    ...
    public function scoreAndRewardStatusZero($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_20solt_score_reward_status_zero');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function grandRewardZero($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_20solt_big_reward_status_zero');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function smallRewardZero($ip, $port)
    {
        ...
        while(!$result && $count < 5 ) {
            ...
            logger('v1_20solt_small_reward_status_zero');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function drawCardStop123Red7($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_20solt_draw_card_stop_123_red_7');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function drawCardStop123Blue7($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_20solt_draw_card_stop_123_blue_7');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function drawCardStop123Bar($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_20solt_draw_card_stop_123_bar');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function drawCardStop321Red7($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_20solt_draw_card_stop_321_red_7');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function drawCardStop321Blue7($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_20solt_draw_card_stop_321_blue_7');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function drawCardStop321Bar($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_20solt_draw_crad_stop_321_bar');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function sevenDisplay($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_20solt_seven_display');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function big_reward_status($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_20solt_big_reward_status');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function small_reward_status($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_20solt_samll_reward_status');
            logger('ip:'.$ip);
            logger('port:'.$port);
