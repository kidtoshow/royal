...
class V4SlotController extends Controller
{
    //取值韋峯部分
    // 洗分控制
    ...
    public function washPointControl($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v4_slot_wash_point_control');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function openPointControl($ip, $port)
    {
        ...
        while (!$result && $count < 5) {
            ...
            logger('v4_slot_open_point_control');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function openPointStatus($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v4_slot_open_point_status');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function openPointZero($ip, $port)
    {
        ...
        while (!$result && $count < 5){
            ...
            logger('v4_slot_open_point_zero');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function checkAutoCard($ip, $port)
    {
        $result = 0;
        $count = 0;
        $crc = Resource::calculateCrc(self::CHECK_AUTO_CARD.'0000');
        while(!$result && $count < 5) {
            ...
            logger('v4_slot_check_auto_card'.PHP_EOL);
    ...
    public function startAuto($ip, $port)
    {
        $result = 0;
        $count = 0;
        $crc = Resource::calculateCrc(self::START_AUTO.'0000');
        while(!$result && $count < 5) {
            ...
            logger('v4_slot_start_auto');
            logger('ip:'.$ip);
            logger('port:'.$port);
            logger('指令：'.self::START_AUTO . $crc);
            logger('回傳值：'.$return);
            logger('time:'.Carbon::now()->format('Y-m-d H:i:s:v'));
            // $check = Resource::calculateCrc($return, 6);
    ...
    public function stopAuto($ip, $port)
    {
        $result = 0;
        $count = 0;
        $crc = Resource::calculateCrc(self::STOP_AUTO.'0000');
        while(!$result && $count < 5) {
    ...
    public function stakePoint($ip, $port)
    {
        $result = 0;
        $count = 0;
        $crc = Resource::calculateCrc(self::STAKE_POINT.'0000');
    ...
    public function startGame($ip, $port)
    {
        $result = 0;
        $count = 0;
        $crc = Resource::calculateCrc(self::GAME_START.'0000');
        while(!$result && $count < 5) {
    ...
    public function stop1($ip, $port)
    {
        $result = 0;
        $count = 0;
        $crc = Resource::calculateCrc(self::STOP_1.'0000');
        while(!$result && $count < 5) {
    ...
    public function stop2($ip, $port)
    {
        $result = 0;
        $count = 0;
        $crc = Resource::calculateCrc(self::STOP_2.'0000');
        while(!$result && $count < 5) {
    ...
    public function stop3($ip, $port)
    {
        $result = 0;
        $count = 0;
        $crc = Resource::calculateCrc(self::STOP_3.'0000');
        while(!$result && $count < 5) {
    ...
    public function scoreAndRewardStatusZero($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v4_slot_score_reward_status_zero');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function grandRewardZero($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v4_slot_big_reward_status_zero');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function smallRewardZero($ip, $port)
    {
        ...
        while(!$result && $count < 5 ) {
            ...
            logger('v4_slot_small_reward_status_zero');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function drawCardStop123Red7($ip, $port)
    {
        $result = 0;
        $count = 0;
        $crc = Resource::calculateCrc(self::STOP_123_RED_7.'0000');
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::START_AUTO);
            logger('v4_slot_draw_card_stop_123_red_7');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function drawCardStop123Blue7($ip, $port)
    {
        $result = 0;
        $count = 0;
        $crc = Resource::calculateCrc(self::STOP_123_BLUE_7.'0000');
        while(!$result && $count < 5) {
    ...
    public function drawCardStop321Red7($ip, $port)
    {
        $result = 0;
        $count = 0;
        $crc = Resource::calculateCrc(self::STOP_321_RED_7.'0000');
        while(!$result && $count < 5) {
    ...
    public function drawCardStop321Blue7($ip, $port)
    {
        $result = 0;
        $count = 0;
        $crc = Resource::calculateCrc(self::STOP_321_BULE_7.'0000');
        while(!$result && $count < 5) {
            ...
            // $return = Resource::send_socket($ip, $port, self::START_AUTO);
            logger('v4_slot_draw_card_stop_321_blue_7');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function big_reward_status($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v4_slot_big_reward_status');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function small_reward_status($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v4_slot_samll_reward_status');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function bigRewardCount($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v4_slot_big_reward_count');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function smallRewardCount($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v4_slot_small_reward_count');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function bigRewardCountRest($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v4_slot_big_reward_count_reset');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function smallRewardCountRest($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v4_slot_small_reward_count_reset');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function coinInCountReset($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v4_slot_coin_in_count_reset');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function coinOutCountReset($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v4_slot_coin_out_count_reset');
            logger('ip:'.$ip);
            logger('port:'.$port);
