...
class V1ClownController extends Controller
{
    //取值韋峯部分
    // 洗分控制
    CONST WASH_POINT_CONTROL = '01 05 08 79 FF 00 5F 83';
    // 壓分
    CONST OPEN_POINT_AMOUNT = '01 06 00 E7 %04s';
    CONST OPEN_POINT_SPEED = '01 06 00 F1 00 0A 58 3E';
    // 開分控制
    CONST OPEN_POINT_CONTROL = '01 05 08 71 FF 00 DE 41';
    // 投幣ＯＮ
    CONST COIN_ON = '01 05 08 84 FF 00 CE 73';
    // 投幣ＯＦＦ
    CONST COIN_OFF = '01 05 08 84 00 00 8F 83';
    // 大獎狀態
    ...
    //搖桿_短
    CONST JOYSTICK_SHORT = '01 05 08 8C FF 00 4F B1';
    //搖桿_長
    // CONST JOYSTICK_LONG = '01 05 08 8D FF 00 1E 71'; //2秒後停
    CONST JOYSTICK_LONG = '01 05 08 8E FF 00 EE 71';
    CONST JOYSTICK_LONG_STOP = '01 05 08 8E 00 00 AF 81';
    //停1
    CONST STOP_1 = '01 05 08 96 FF 00 6E 76';
    //停2
    CONST STOP_2 = '01 05 08 A0 FF 00 8E 78';
    //停3
    CONST STOP_3 = '01 05 08 AA FF 00 AE 7A';
    ...
    public function washPointControl($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('V1_clown_wash_point_control');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function openPointControl($ip, $port)
    {
        ...
        while (!$result && $count < 5) {
            ...
            logger('v1_clown_open_point_control');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function openPointStatus($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_clown_open_point_status');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function openPointZero($ip, $port)
    {
        ...
        while (!$result && $count < 5){
            ...
            logger('v1_clown_open_point_zero');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function scoreAndRewardStatusZero($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_clown_score_reward_status_zero');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function grandRewardZero($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_clown_big_reward_status_zero');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function smallRewardZero($ip, $port)
    {
        ...
        while(!$result && $count < 5 ) {
            ...
            logger('v1_clown_small_reward_status_zero');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function bigRewardStatus($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_clown_big_reward_status');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function smallRewardStatus($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_clown_samll_reward_status');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function joystickShort($ip, $port)
    {
        ...
        while(!$result && $count < 5) {
            ...
            logger('v1_clown_joystick_short');
            logger('ip:'.$ip);
            logger('port:'.$port);
    ...
    public function joystickLong($ip, $port)
    {
        ...
        while(!$result_off && $count_off < 5) {
            ...
            logger('v1_clown_joystick_long_stop');
            logger('ip:'.$ip);
            logger('port:'.$port);
